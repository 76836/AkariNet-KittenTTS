...
const TTSProcessor = {
    ...
    async init(src = 'https://76836.github.io/AkariNet-KittenTTS/tts-cleaner.js') {
        ...
        return new Promise((resolve, reject) => {
            ...
            script.onerror = () => reject(new Error('Failed to load TTS library'));
            document.head.appendChild(script);
        });
    },

    parse(input) {
        // Fallback if library isn't available
        if (!window.TTSCleaner) return input;

        const { cleanMarkdownForTTS } = window.TTSCleaner;
        const removeEmojis = localStorage.getItem('removeEmojis') === 'true';
        return cleanMarkdownForTTS(input, {}, { removeEmojis });
    }
};
TTSProcessor.init(); // Starts loading immediately
...
const soundEffect = {
  shape: [0,0,2,5,9,13,14,14,13,9,5,2,0,0,0,0,0,0,0,0],
  make: () => {
    const activeDur = 2.6, silenceDur = 1.8, cycle = activeDur + silenceDur;
    const rate = 0.2;
    let t = 0;
    return {
      next(audioTime) {
        const phase = t % cycle;
        if (phase < activeDur) {
          const p = phase / activeDur;
          const env = Math.sin(p * Math.PI);
          // Internal tick logic for Web Audio
          playTick(audioTime, env);
          t += rate;
          return { nextTime: audioTime + rate };
        } else {
          const remaining = cycle - phase;
          t += remaining;
          return { nextTime: audioTime + remaining };
        }
      }
    };
  }
...
function playTick(time, volume) {
  if (!audioCtx) return;
  const osc = audioCtx.createOscillator();
  const gain = audioCtx.createGain();

  osc.type = 'sine';
  osc.frequency.setValueAtTime(440, time); // Adjust frequency as needed
...
function startSound() {
  ...
  isPlaying = true;
  const generator = soundEffect.make();

  function loop(time) {
    if (!isPlaying) return;
    const result = generator.next(time);
    const delay = (result.nextTime - time) * 1000;
    nextTimeout = setTimeout(() => loop(result.nextTime), delay);
  }
...
window.initTTS = (config) => {
    ...
    window.audibleThinking = ()     => startSound();
